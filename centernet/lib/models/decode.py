...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    ...
    print('batch: ', batch, ' cat: ', cat, ' height: ', height, ' width: ', width)
    ...
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    # print(scores.shape)
    # print(type(scores),type(clses),type(ys),type(xs))
    # print('classes: ', clses)
    # numclses = clses.cpu().numpy()
    # numclses = clses
    # print("num: ", numclses.shape)
    # print(numclses)
    
    onlyperson = 1
    
    #print("fsadfsaf: ", type(clses), type(xs))
    #print("fsadfsaf: ", clses)
    
    perclses = ((clses[0]==0).nonzero())
    
    if(perclses.shape[0]==0):
    #to avod empty error when no human retrn 1 virtual human of  confidence
      a = torch.tensor([[[  0,  0, 1, 1,   0.0, 0.0000]]],device='cuda:0')
      return a
    
    if(onlyperson):
      perclses = ((clses[0]==0).nonzero()).view(1,-1)
      
      # print(perclses) 
      # print("xs: ", xs.shape)
      # print(xs)
      
      xs = xs.gather(1, perclses)
      clses = clses.gather(1, perclses)
      ys = ys.gather(1, perclses)
      inds = inds.gather(1, perclses)
      scores = scores.gather(1, perclses)
      
      K = len(xs[0])
      
      # print(xs)
      # print("lenxs: ", len(xs[0]))# print(reg[0].shape)
